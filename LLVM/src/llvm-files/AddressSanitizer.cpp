...
#include <cxxabi.h>
#include <stdlib.h>
#include "llvm/Support/CMASanInfoParser.h"
#endif
...
#ifndef DISABLE_CMASAN
// CMASan
static const char *kCMASizeGetterFName = "__asan_getCMAChunkSize";
static const char *kReallocateCMAChunkStartFName = "__asan_reallocateCMAChunkStart";
static const char *kReallocateCMAChunkEndFName = "__asan_reallocateCMAChunkEnd";
static const char *kAllocateCMAChunkFName = "__asan_allocateCMAChunk";
static const char *kReleaseCMAChunkFName = "__asan_releaseCMAChunk";
static const char *kIsInOtherCMAFName = "__asan_isInOtherCMA";
static const char *kCMAExtendSizeFName = "__asan_extendSize";
static const char *kLogCMARelatedFName = "__asan_log_cma_related_function";
static const char *kCMAEntryBlockName = "cma.entry";
static const char *kCMAReturnBlockName = "cma.return";
static const char *kCMAClearCMAChunksFName = "__asan_clearCMAChunks";
static const char *kCMAEmptyQuarantineOnGC = "__asan_emptyQuarantineOnGC";
static const char *kCMAInitMmapRangeFName = "__asan_initMmapRange";
static const char *kCMAFlushMmapRangeFName = "__asan_flushMmapRange";

static cl::opt<std::string> CMASanFilePath(
    "asan-cma-writeback-path", cl::desc("Path for CMASan suppression info"),
    cl::Optional, cl::init(""));
#endif
...
private:
  ...
  Type *IntPtrTy;
  Type *IntTy;
  std::vector<const Function*> InstrumentedCMAs;
  
  static char *GetDemangledFunctionName (StringRef functionName);

  /*
   * From llvm/CodeGen/IntrinsicLowering.h
   */
  template <class ArgIt>
  static CallInst *CreateNewCIBeforeInst (Module *M, IRBuilder<> &IRB, StringRef NewFn,
                                      ArgIt ArgBegin, ArgIt ArgEnd, Type *RetTy);
  void ReplaceCallInst (CallInst *newCI, CallInst *oldCI);
  bool InstrumentTargetFunction (Function *F, CMAInfo info);
  void InstrumentMmapFamily(CallInst *CI, bool isMmap);

  template <class ArgIt>
  SmallVector<Value *, 1> generateNewArgs (ArgIt originBegin, ArgIt originEnd,
                                           Value *newValue, int targetIdx);
  template <class ArgIt>
  SmallVector<Value *, 1> generateNewArgs (ArgIt originBegin, ArgIt originEnd,
                                           Value *newValue1, int targetIdx1,
                                           Value *newValue2, int targetIdx2);
  Value* CreateBitCastOnTypeMismatch(IRBuilder<> &IRB, Value* newV, Value* orgV);
  Value* getCMAQuarantineZoneID(IRBuilder<> &IRB, Function *F, CMAInfo info);

public:
  // static bool IsCMAFn (Function* F, CMAInfo& info);
  CMASanitizer (Module &M);
  bool InstrumentCMA (Module &M);
};

CMASanitizer::CMASanitizer (Module &M)
{
  C = &(M.getContext ());
  LongSize = M.getDataLayout ().getPointerSizeInBits ();
  IntTy = Type::getIntNTy (*C, LongSize);
  IntPtrTy = Type::getIntNPtrTy (*C, LongSize);
  // cmaInfos = getCMAInfo ();
}

char *
CMASanitizer::GetDemangledFunctionName (StringRef mangledFunctionName)
{
  int demangledStatus;
  char *functionName;
  ...
  char *demangledFunctionName
      = abi::__cxa_demangle (functionName, nullptr, nullptr, &demangledStatus);
  if (demangledStatus == 0) functionName = demangledFunctionName;

  return functionName;
}

template <class ArgIt>
CallInst *CMASanitizer::CreateNewCIBeforeInst (Module *M, IRBuilder<> &IRB, StringRef NewFn,
                                      ArgIt ArgBegin, ArgIt ArgEnd, Type *RetTy)
{
  ...
  for (ArgIt A = ArgBegin; A != ArgEnd; ++A)
    ParamTys.push_back ((*A)->getType ());
  FunctionCallee FCache = M->getOrInsertFunction (
      NewFn, FunctionType::get (RetTy, ParamTys, false));

  SmallVector<Value *, 8> Args (ArgBegin, ArgEnd);
  CallInst *NewCI = IRB.CreateCall (FCache, Args);
...
void
CMASanitizer::ReplaceCallInst (CallInst *newCI, CallInst *oldCI)
{
...
// Change the value at the target index as new value.
// If the range is smaller than target index, it returns same copied Vector
// object.
template <class ArgIt>
SmallVector<Value *, 1>
CMASanitizer::generateNewArgs (ArgIt originBegin, ArgIt originEnd,
                               Value *newValue, int targetIdx)
{
  SmallVector<Value *, 1> newArgs;

  for (auto arg_iter = originBegin, argEnd = originEnd; arg_iter != argEnd;
       ++arg_iter)
    {
      if (arg_iter == originBegin + targetIdx)
        newArgs.push_back (newValue);
      else
        newArgs.push_back (*arg_iter);
    }
                               ...
                               Value *newValue2, int targetIdx2)
{
  ...
  for (auto arg_iter = originBegin, argEnd = originEnd; arg_iter != argEnd;
       ++arg_iter)
    {
      if (arg_iter == originBegin + targetIdx1)
        newArgs.push_back (newValue1);
      else if (arg_iter == originBegin + targetIdx2)
        newArgs.push_back (newValue2);
      else
        newArgs.push_back (*arg_iter);
    }
...
// bool
// CMASanitizer::IsCMAFn (Function *F, CMAInfo& info)
// {
//   StringRef CIFunctionName = F->getName ();

//   StringRef demangledFunctionName
//       = StringRef (GetDemangledFunctionName (CIFunctionName));

//   if ((demangledFunctionName.find ("llvm.") != std::string::npos)
//       || (demangledFunctionName.find ("__asan") != std::string::npos)
//       || (demangledFunctionName.find ("__sanitizer") != std::string::npos))
//     return false;


//   if (demangledFunctionName.equals (GetDemangledFunctionName(info.functionName))){
//     return true;
//   }
//   return false;
// }
...
Value*
CMASanitizer::CreateBitCastOnTypeMismatch(IRBuilder<> &IRB, Value* newV, Value* orgV) {
  Value* result = newV;
  if (newV->getType() != orgV->getType()) {
    result = IRB.CreateBitCast(newV, orgV->getType());
  }
  return result;
}

// Id specification based on the first parameter
Value* CMASanitizer::getCMAQuarantineZoneID(IRBuilder<> &IRB, Function *F, CMAInfo info) {
  if (info.idIdx != -1) return F->getArg(info.idIdx); // idIdx has priority than family id
  if (info.id != -1) return ConstantInt::get(IntTy, info.id);
  if (info.ptrIdx == 0 || info.sizeIdx == 0 || F->args().empty()) return ConstantInt::get(IntTy, 1); // == nullptr

  Value *FirstArg = F->getArg(0);
  Type *ArgType = FirstArg->getType();
  if (ArgType->isPointerTy()) return IRB.CreatePtrToInt(FirstArg, IntTy);
  if (ArgType->isIntegerTy()) return IRB.CreateBitCast(FirstArg, IntTy);

  llvm::errs() << "Warning: first parameter is neither pointer type or integer type, CMASan will use only one CMA quarantine zone\n";
  return ConstantInt::get(IntTy, 1);
}

bool
CMASanitizer::InstrumentTargetFunction (Function *F, CMAInfo info)
{
  ...
  StringRef demangledFunctionName = StringRef (GetDemangledFunctionName (FName));

  // Class Member Function (deprecated, will be done in static analysis)
  // if (!info.isMemberFunction
  //     && demangledFunctionName.contains (StringRef ("::")))
  // {
  ...
  llvm::errs () << "------------------ Instrumenting " << demangledFunctionName  <<  " ------------------\n";

  Value *IsInCMAArg, *SizeArg, *PtrInArg, *PtrOutArg, *GranularityArg, *IdArg;

  //* Define Entry point *//
  BasicBlock &EntryBB = F->getEntryBlock();
  IRBuilder<> IRBEntry(dyn_cast<Instruction>(EntryBB.getFirstInsertionPt()));
  BasicBlock *CheckerBB;

  if (info.type == LOG_TARGET) {
    SmallVector<Value *, 1> EmptyArgs = { };
    CreateNewCIBeforeInst (M, IRBEntry, kLogCMARelatedFName, EmptyArgs.begin (), EmptyArgs.end (), Type::getVoidTy(F->getContext()));
    return true;
  }

  if (info.type == FREE) {
    CheckerBB = BasicBlock::Create(F->getContext(), kCMAEntryBlockName, F, &EntryBB);
    IRBEntry.SetInsertPoint(CheckerBB);
  }

  //* Extract informations from parameters *//
  // Intercept function argument
  if (info.sizeIdx != -1) {
    SizeArg = F->getArg(info.sizeIdx);
  }
  if (info.ptrIdx != -1) {
    PtrInArg = F->getArg(info.ptrIdx);
  }

  if (info.type == ALLOC  || info.type == CALLOC || info.type == FREE || info.type == REALLOC || info.type == CLEAR || info.type == GC) {
    IdArg = getCMAQuarantineZoneID(IRBEntry, F, info);
  }

  //* Instrument Entry Point *//
  // Construct Stack and Check if callstack contains CMA
  if (info.type != SIZE) {
    SmallVector<Value *, 1> EmptyArgs = { };
    IsInCMAArg = CreateNewCIBeforeInst (M, IRBEntry, kIsInOtherCMAFName, EmptyArgs.begin (), EmptyArgs.end (), Type::getInt1Ty(F->getContext()));
  }

  if (info.type == ALLOC || info.type == REALLOC || info.type == CALLOC)
  {
    if (info.sizeIdx == -1) {
      errs() << "fatal: size index is not specified in ALLOC/REALLOC/CALLOC\n";
      return false;
    }

    if (info.type == CALLOC) { // CALLOC: Inspect size from its num argument
      if (info.numIdx == -1) {
        errs() << "fatal: num index is not specified in CALLOC\n";
        return false;
      }
      GranularityArg = F->getArg(info.numIdx);
    } else { // ALLOC/REALLOC: Inspect size granularity based on its return type
      Type* retTy = F->getReturnType();
       const llvm::DataLayout& DL = F->getParent()->getDataLayout();
      if (retTy->isPointerTy()) {
        if (retTy->getPointerElementType()->isStructTy()) {
          uint64_t structSize = DL.getTypeAllocSize(retTy->getPointerElementType());
          GranularityArg = ConstantInt::get(IntTy, structSize);
        } else {
          GranularityArg = ConstantInt::get(IntTy, DL.getTypeSizeInBits(retTy->getPointerElementType()) / 8);
        }
      } else if (retTy->isIntegerTy()) {
        GranularityArg = ConstantInt::get(IntTy, DL.getTypeSizeInBits(retTy) / 8);
      } else {
        llvm_unreachable("ALLOC/REALLOC has an unsupported type\n");
      }
    }

    SmallVector<Value *, 1> SizeArgs = { SizeArg, GranularityArg, IsInCMAArg };
    CallInst *ExtendedSizeCI = CreateNewCIBeforeInst (M, IRBEntry, kCMAExtendSizeFName, SizeArgs.begin (), SizeArgs.end (), SizeArg->getType ());
    Value* ExtendedSize = CreateBitCastOnTypeMismatch(IRBEntry, ExtendedSizeCI, SizeArg);
    SizeArg->replaceAllUsesWith(ExtendedSize); // Replace all uses in CMA Function
    ExtendedSizeCI->setArgOperand(0, SizeArg); // Fill the original size back to the size arg of extendSize

    errs () << "Size was extended!\n";
    if (info.type == REALLOC)
    {
      SmallVector<Value *, 1> argsForStart = { PtrInArg, IsInCMAArg, IdArg };
      CallInst* OldCMAChunkOrNullCI = CreateNewCIBeforeInst (M, IRBEntry, kReallocateCMAChunkStartFName,
          argsForStart.begin (), argsForStart.end (), Type::getVoidTy (F->getContext()));
      // Value* OldCMAChunkOrNull = CreateBitCastOnTypeMismatch(IRBEntry, OldCMAChunkOrNullCI, PtrInArg);
      // PtrInArg->replaceAllUsesWith(OldCMAChunkOrNull);
      // OldCMAChunkOrNullCI->setArgOperand(0, PtrInArg);
      // PtrOutArg = OldCMAChunkOrNullCI; // from quarantine zone, or ptrIn
    }
  } else if (info.type == FREE)
  {
    if (info.ptrIdx == -1) {
      errs() << "fatal: ptr index is not specified in FREE\n";
      return false;
    }

    // Modify redzone, inspect whether the call is needed (NULL check)
    SmallVector<Value *, 1> PtrInArgs = { PtrInArg, IdArg, IsInCMAArg };
    CallInst* ReleasedCMAChunkOrNullCI = CreateNewCIBeforeInst (M, IRBEntry, kReleaseCMAChunkFName,
        PtrInArgs.begin (), PtrInArgs.end (), IntPtrTy);
    Value* ReleasedCMAChunkOrNull = CreateBitCastOnTypeMismatch(IRBEntry, ReleasedCMAChunkOrNullCI, PtrInArg);
    PtrInArg->replaceAllUsesWith(ReleasedCMAChunkOrNull);
    ReleasedCMAChunkOrNullCI->setArgOperand(0, PtrInArg);

    // Check whether pointer is null (in quarantine zone)
    if ((F->getReturnType()->isVoidTy() || F->getReturnType()->isIntegerTy(8) || dyn_cast<PointerType>(F->getReturnType()))) {
      BasicBlock *ReturnTrueBB = BasicBlock::Create(F->getContext(), kCMAReturnBlockName, F, &EntryBB);
      // Null check and branch
      Value* FuncPtrInArg = ReleasedCMAChunkOrNull;
      Value* IsPtrNull = IRBEntry.CreateIsNull(FuncPtrInArg);
      IRBEntry.CreateCondBr(IsPtrNull, ReturnTrueBB, &EntryBB);

      // Return
      IRBEntry.SetInsertPoint(ReturnTrueBB);
      if (F->getReturnType()->isVoidTy()) {
        IRBEntry.CreateRetVoid();
      } else if (F->getReturnType()->isIntegerTy(8)) {
        IRBEntry.CreateRet(ConstantInt::getTrue(Type::getInt8Ty(F->getContext())));
      } else {
        IRBEntry.CreateRet(ConstantPointerNull::get(cast<PointerType>(F->getReturnType())));
      }
      // errs () << "Return Early\n";
    } else {
      IRBEntry.CreateBr(&EntryBB);
    }
    ...
    SmallVector<Value *, 1> ClearArgs = { IdArg };
    CreateNewCIBeforeInst (M, IRBEntry, kCMAClearCMAChunksFName, ClearArgs.begin (), ClearArgs.end (), Type::getVoidTy (F->getContext ()));

    errs () << "SUCCESS\n";
    return true;
  } else if (info.type == GC) {
    SmallVector<Value *, 1> GCArgs = { IdArg };
    CreateNewCIBeforeInst (M, IRBEntry, kCMAEmptyQuarantineOnGC, GCArgs.begin (), GCArgs.end (), Type::getVoidTy (F->getContext ()));
    errs () << "SUCCESS\n";
    return true;
  }
  ...
  SmallVector<ReturnInst*, 1> ReturnInsts;
  for (Function::iterator BBI = F->begin (), BBE = F->end (); BBI != BBE; ++BBI)
  {
    for (BasicBlock::iterator II = BBI->begin (), IE = BBI->end (); II != IE;
        ++II)
      {
        if (ReturnInst *RI = dyn_cast<ReturnInst> (II)) ReturnInsts.push_back(RI);
      }
  }
  ...
  if (info.type == SIZE) {
    for (ReturnInst *RI : ReturnInsts) {
      IRBuilder<> IRBExit(RI);
      // Create CMA Size getter with the return value of size getter candidate
      SmallVector<Value *, 1> GetArgs = { PtrInArg, RI->getOperand(0) };
      CallInst *SizeGetterCI = CreateNewCIBeforeInst (M, IRBExit, kCMASizeGetterFName, GetArgs.begin (),
                               GetArgs.end (), F->getReturnType ());
      Value* SizeGetter = CreateBitCastOnTypeMismatch(IRBExit, SizeGetterCI, RI->getOperand(0));
      RI->setOperand(0, SizeGetter);
    }
  } else if (info.type == ALLOC || info.type == CALLOC) {
    for (ReturnInst *RI : ReturnInsts) {
      IRBuilder<> IRBExit(RI);
      SmallVector<Value *, 1> PoisonArgs = { RI->getOperand(0), SizeArg, GranularityArg, IdArg, IsInCMAArg };
      CreateNewCIBeforeInst (M, IRBExit, kAllocateCMAChunkFName, PoisonArgs.begin (), PoisonArgs.end (), Type::getVoidTy (F->getContext ()));
    }
  } else if (info.type == REALLOC) {
    // Instrument after REALLOC
    for (ReturnInst *RI : ReturnInsts) {
      IRBuilder<> IRBExit(RI);
      SmallVector<Value *, 1> ArgsForEnd = { PtrInArg, RI->getOperand(0), SizeArg, GranularityArg, IdArg, IsInCMAArg };
      CreateNewCIBeforeInst (M, IRBExit, kReallocateCMAChunkEndFName, ArgsForEnd.begin (), ArgsForEnd.end (), Type::getVoidTy (F->getContext ()));
    }
  } else {
    errs () << "FAILED: undefined type\n";
    return false;
  }
...
void
CMASanitizer::InstrumentMmapFamily(CallInst *CI, bool isMmap) {
  IRBuilder<> IRB(CI->getNextNode());
  Value *SizeArg = dyn_cast<Value>(CI->arg_begin()+1);
  Value *PtrArg = dyn_cast<Value>(CI->arg_begin());
  if (isMmap) { // only for logging
    llvm::errs () << "------------------ Instrumenting mmap ------------------\n";
    SmallVector<Value *, 1> Args = { CI, SizeArg };
    CreateNewCIBeforeInst (CI->getModule(), IRB, kCMAInitMmapRangeFName, Args.begin (), Args.end (), Type::getVoidTy(CI->getContext()));
  } else {
    llvm::errs () << "------------------ Instrumenting munmap ------------------\n";
    SmallVector<Value *, 1> Args = { PtrArg, SizeArg };
    CreateNewCIBeforeInst (CI->getModule(), IRB, kCMAFlushMmapRangeFName, Args.begin (), Args.end (), Type::getVoidTy(CI->getContext()));
  }
}

bool
CMASanitizer::InstrumentCMA (Module &M)
{
  CMASanParseInfo(M);
  for (Module::iterator FI = M.begin (), FE = M.end (); FI != FE; ++FI)
  {
    Function * FF = dyn_cast<Function>(FI);    
    if (FF && !FF->isDeclaration() && FF->hasFnAttribute(kCMASanTargetAttribute) && !FF->getMetadata("cma.function.instrumented")) {
      if (InstrumentTargetFunction (FF, getCMAInfo(FF))) {
        LLVMContext &C = FF->getContext();
        MDNode *MD = MDNode::get (M.getContext (), MDString::get (C, "1"));
        FF->setMetadata ("cma.function.instrumented", MD);
        InstrumentedCMAs.push_back(FF);
      }
    }

    for (Function::iterator BI = FF->begin (), BE = FF->end (); BI!= BE; ++BI) {
      for (BasicBlock::iterator II = BI->begin (), IE = BI->end (); II!= IE; ++II) {
        if (CallInst *CI = dyn_cast<CallInst> (II)) {
          Function *CalledFunction = CI->getCalledFunction();
          if (CalledFunction) {
            StringRef FName = CalledFunction->getName();
            if (FName == StringRef("munmap")) {
              InstrumentMmapFamily(CI, false);
            } 
#ifdef EXPERIMENT_COVERAGE_INCREMENT
            else if(FName == StringRef("mmap") || FName == StringRef("mmap64")) {
              InstrumentMmapFamily(CI, true);
            }
#endif
          }
        }
  ...
  WriteBackCMANames(CMASanFilePath, M, InstrumentedCMAs);
  return true;
}
#endif
...
#ifndef DISABLE_CMASAN
  CMASanitizer cmasan(M);
  cmasan.InstrumentCMA(M);
#endif
