...
#ifndef DISABLE_CMASAN                              
  if (tlt_chunk_info* chunk_info = tlt_get(addr)) { // safe for synchronization we locked before (at ErrorGeneric, ReportDoubleFree)
    // GetAccessToHeapChunkInformation
    ChunkAccess *access_descr = &descr->chunk_access;
    access_descr->bad_addr = addr;
    
    uptr chunk_size = chunk_info->chunk_size;
    uptr chunk_begin = chunk_info->start_addr;
    uptr chunk_end = chunk_begin + chunk_size;
    access_descr->offset = addr - chunk_begin;
    
    if (addr >= chunk_begin) {
      if (addr >= chunk_end) {
        access_descr->access_type = kAccessTypeRight;
      } else {
        access_descr->access_type = kAccessTypeInside;
      } 
    } else {
      access_descr->access_type = kAccessTypeUnknown;
    }
    
    access_descr->chunk_begin = chunk_begin;
    access_descr->chunk_size = chunk_size;
    access_descr->user_requested_alignment = chunk_begin % ASAN_SHADOW_GRANULARITY;
    access_descr->alloc_type = 0;
    descr->addr = addr;
    // Get tid and sid
    CHECK_NE(chunk_info->alloc_thread_id, kInvalidTid);
    descr->alloc_tid = chunk_info->alloc_thread_id;
    descr->alloc_stack_id = chunk_info->alloc_stack_id;
    descr->free_tid = chunk_info->free_thread_id;
    if (descr->free_tid != kInvalidTid)
      descr->free_stack_id = chunk_info->free_stack_id;
    return true;
  }
#endif
