...
ErrorGeneric::ErrorGeneric(u32 tid, uptr pc_, uptr bp_, uptr sp_, uptr addr,
  ...
  if (access_size) {
    ...
    #ifndef DISABLE_CMASAN
      tlt_chunk_info *chunk_info = tlt_get(addr);
    #endif
      switch (shadow_val) {
        ...
        #ifndef DISABLE_CMASAN
          if (chunk_info != 0 && chunk_info->status==ALLOCATED) {
            bug_descr = "heap-buffer-overflow-from-CMA";
          }
        #endif
          break;
        ...
        #ifndef DISABLE_CMASAN
          if (chunk_info !=0 && chunk_info->status==DEALLOCATED)
            bug_descr = "heap-use-after-free-from-CMA";
        #endif
          break;
...
#ifndef DISABLE_CMASAN
static void
PrintLegendCMA (InternalScopedString *str)
{
  str->append ("Shadow byte legend (one shadow byte represents %d "
               "application bytes):\n",
               (int)ASAN_SHADOW_GRANULARITY);
  PrintShadowByte (str, "  Addressable:           ", 0);
  str->append ("  Left Partially addressable (0x#0): ");
  for (u8 i = 1; i < ASAN_SHADOW_GRANULARITY; i++)
    PrintShadowByte (str, "", i << 4, " ");
  str->append ("\n");
  str->append ("  Right Partially addressable (0x0#): ");
  for (u8 i = 1; i < ASAN_SHADOW_GRANULARITY; i++)
    PrintShadowByte (str, "", i, " ");
  str->append ("\n");
  PrintShadowByte (str, "  Heap left redzone:       ", kAsanHeapLeftRedzoneMagic);
  PrintShadowByte (str, "  Freed heap region:       ", kAsanHeapFreeMagic);
  PrintShadowByte (str, "  Non-CMA region:          ", 0xfe);
  str->append ("\n");
}
#endif
...
#ifndef DISABLE_CMASAN
static void
PrintCMABytes (InternalScopedString *str, const char *before, uptr bytes,
                  uptr guilty, uptr n)
{
  Decorator d;
  if (before)
    str->append ("%s%p:", before, bytes);
  for (uptr i = 0; i < n; i += 8)
    {
      uptr p = RoundDownTo(bytes, ASAN_SHADOW_GRANULARITY) + i;
      const char *before = p == guilty                   ? "["
                           : (p - 8 == guilty && i != 0) ? ""
                                                         : " ";
      const char *after = p == guilty ? "]" : "";

      if (tlt_chunk_info* info=tlt_get(p)) {
        if (info->status == DEALLOCATED) {
          PrintMemoryByte (str, before, 0xfd, true, after);
        } else if (p < info->start_addr) { // chunk is non-aligned, just think as aligned
          uptr addressable = ASAN_SHADOW_GRANULARITY - (info->start_addr - p); // < 7 is guaranteed
          if (addressable > info->chunk_size) addressable = info->chunk_size;
          PrintMemoryByte (str, before, addressable << 4, true, after); // express non aligned offset with 0x[]0
        } else { // p >= info->start_addr
          uptr current_chunk_offset = p - info->start_addr; 
          uptr chunk_size, rz_size;
          if (current_chunk_offset < info->chunk_size) {
            uptr remains = info->chunk_size - current_chunk_offset;
            if (remains < ASAN_SHADOW_GRANULARITY) {
              PrintMemoryByte (str, before, remains, true, after);
            } else {
              PrintMemoryByte (str, before, 0x00, true, after);
            }
          } else { // in rz
            PrintMemoryByte (str, before, 0xfa, true, after);
          }
        }
      } else {
        PrintMemoryByte (str, before, 0xfe, true, after);
      }
    }
  str->append ("\n");
}
...
#ifndef DISABLE_CMASAN
  str.append ("\n");
  str.append ("CMASan tracking info around the buggy address:\n");
  uptr cma_n_bytes_per_row = n_bytes_per_row << 3;
  uptr aligned_addr = RoundDownTo(addr, ASAN_SHADOW_GRANULARITY);
  for (int i = -5; i <= 5; i++)
    {
      uptr row_shadow_addr = aligned_shadow + i * n_bytes_per_row;
      uptr row_cma_addr = ShadowToMem(row_shadow_addr);
      
      const char *prefix = (i == 0) ? "=>" : "  ";

      PrintCMABytes (&str, prefix, row_cma_addr, aligned_addr,
                        cma_n_bytes_per_row);
    }
  PrintLegendCMA(&str);
#endif
