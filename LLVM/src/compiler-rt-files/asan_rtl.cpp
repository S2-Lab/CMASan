...
#define CMASAN_LOG_CHECK_FUNC(addr, is_write, size, exp_arg) \
  CMASanLogCheck(addr, is_write, size, exp_arg); 
#else
#define CMASAN_LOG_CHECK_FUNC(addr, is_write, size, exp_arg)
#endif
...
#define ASAN_MEMORY_ACCESS_CALLBACK_BODY(type, is_write, size, exp_arg, fatal) \
  ...
  CMASAN_LOG_CHECK_FUNC(addr, is_write, size, exp_arg)                        \
  if (UNLIKELY(s)) {                                                           \
...
#ifndef DISABLE_CMASAN
  CMASAN_LOG_CHECK_FUNC(addr, false, size, 0)   
#endif
...
#ifndef DISABLE_CMASAN
  CMASAN_LOG_CHECK_FUNC(addr, false, size, exp)   
#endif
  if ((addr = __asan_region_is_poisoned(addr, size))) {
    GET_CALLER_PC_BP_SP;
...
#ifndef DISABLE_CMASAN
  CMASAN_LOG_CHECK_FUNC(addr, true, size, 0)   
#endif
  if ((addr = __asan_region_is_poisoned(addr, size))) {
    GET_CALLER_PC_BP_SP;
...
#ifndef DISABLE_CMASAN
  CMASAN_LOG_CHECK_FUNC(addr, true, size, exp)   
#endif
  if ((addr = __asan_region_is_poisoned(addr, size))) {
    GET_CALLER_PC_BP_SP;
